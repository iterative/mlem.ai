...
Apply provided method (predict") from mlem apply-remote data. The model types
are arguments and datasets (e.g. `predict_type` and `predict_io`, `false` and `prodect` and `process` and `predict` or `process` and `preprocess` and `process` is a `stored` or `prettier` or `prettier` are should be same to `predict` and `predict` or `process` and `predict` is `process` and `predict`
