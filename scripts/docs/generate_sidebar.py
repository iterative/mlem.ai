...
def adjust_sidebar(sidebar, to_add: List[str], to_remove: List[str],
                   parent: str = ""):
    res = []
    for entry in sidebar:
        ...
        if "children" in entry:
            parent_path = parent + entry["source"][:-len("/index.md")] if entry[
                "source"] else entry["slug"]
            entry["children"] = adjust_sidebar(entry["children"], to_add,
                                               to_remove, parent_path + "/")
...
def main():
    ...
    md_files = [os.path.relpath(p, DOCS_PATH) for p in
                glob.glob(os.path.join(DOCS_PATH, "**", "*.md"),
                          recursive=True)]
